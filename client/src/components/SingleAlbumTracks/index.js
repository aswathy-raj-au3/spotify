...
const SingleAlbumTracks = ({songs,viewType,fetchAlbumTracksPending,fetchAlbumTracksError, albumName, albums}) => {
  ...
  const renderSongs = () => {
      const selected_album = albums ? albums.filter((item) => item.album.name === albumName) : ""
        ...
        <li className="user-song-item" key={i}>
          ...
          <div className="song-album">
           ...
           <div className="song-added">
            <p>{selected_album[0] ? selected_album[0].album.release_date: "---"}</p>
...
const mapStateToProps = state => {
  return {
    ...
    songs: state.albumTracksReducer.albumTracks ? state.albumTracksReducer.albumTracks: "",
